...
var _pgsql = require('./grammars/pgsql');
...
_pg.types.setTypeParser(20, BigInt);
...
const pgGrammar = new _pgsql2.default();
const db = new _DB2.default(pool, pgGrammar);
...
function runTests() {
    ...
    //     })
    // })
    db.Table('tmp').whereNotIn('id', [1, 5]).orderBy('id').startAt(2).limit(10).select().then(function (res) {
        dd(res);
    });
    // .catch(err => {
    //     dd('ERROR');
